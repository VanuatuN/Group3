...
void force(mdsys_t *sys)
{
    ...
    #if defined(_OPENMP)
    #pragma omp parallel private(i, j, ii, rx, ry, rz, rsq, ffac, r6, rinv) firstprivate(c12,c6,rcsq) reduction(+:epot)
    #endif
    {
    ...
    for (int i = 0; i < (sys->natoms ) - 1; i += step * nthreads)
    {
        #if defined(_MPI)
        #if defined(_OPENMP)
        ii =  i + sys->rank * nthreads + thid;
        #else
        ii =  i + sys->rank;
